...
/**
 * Test case for class RGJL\HotelBooking\Controller\UserController.
 *
...
class UserControllerTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	...
	/**
	 * @var \RGJL\HotelBooking\Controller\UserController
	 */
	protected $subject = NULL;

	protected function setUp() {
		$this->subject = $this->getMock('RGJL\\HotelBooking\\Controller\\UserController', array('redirect', 'forward', 'addFlashMessage'), array(), '', FALSE);
	}
	...
	public function listActionFetchesAllUsersFromRepositoryAndAssignsThemToView() {
		...
		$userRepository = $this->getMock('RGJL\\HotelBooking\\Domain\\Repository\\UserRepository', array('findAll'), array(), '', FALSE);
	...
	public function showActionAssignsTheGivenUserToView() {
		$user = new \RGJL\HotelBooking\Domain\Model\User();
	 ...
	 * @test
	 */
	public function newActionAssignsTheGivenUserToView() {
		$user = new \RGJL\HotelBooking\Domain\Model\User();

		$view = $this->getMock('TYPO3\\CMS\\Extbase\\Mvc\\View\\ViewInterface');
		$view->expects($this->once())->method('assign')->with('newUser', $user);
		$this->inject($this->subject, 'view', $view);
	...
	public function createActionAddsTheGivenUserToUserRepository() {
		...
		$userRepository = $this->getMock('RGJL\\HotelBooking\\Domain\\Repository\\UserRepository', array('add'), array(), '', FALSE);
	...
	public function updateActionUpdatesTheGivenUserInUserRepository() {
		...
		$userRepository = $this->getMock('RGJL\\HotelBooking\\Domain\\Repository\\UserRepository', array('update'), array(), '', FALSE);
	...
	public function deleteActionRemovesTheGivenUserFromUserRepository() {
		...
		$userRepository = $this->getMock('RGJL\\HotelBooking\\Domain\\Repository\\UserRepository', array('remove'), array(), '', FALSE);
